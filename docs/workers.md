...
Use this module when you need to do processing before or without receiving an HTTP call.
...
> **Note:** Future updates will also enable metrics to be served while running.

## Example with initial loader

Set `workers.OnDone` to either `ContinueOnDone` or `ReadyOnDone` to change the behaviour so the process does not exit when the worker is done.

`ReadyOnDone` will keep the readiness probe returning false until the worker is done.
...
| Option            | Description                                                                     |
|-------------------|---------------------------------------------------------------------------------|
| `WithRealPanic()` | Changes the default (handle a panic) into an actual `panic()`                   |
| `WithKV(k, v)`    | Assigns a new key/value to the `context.Context` before starting the worker     |

## Running multiple workers

You can run multiple workers in parallel, and even start a new worker from within an existing one.

The code will wait until all started workers are done before exiting (or performing whatever behaviour is set on the `OnDone` property).